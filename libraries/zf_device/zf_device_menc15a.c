...
//-------------------------------------------------------------------------------------------------------------------
uint16
menc15a_get_absolute_data(menc15a_module_enum menc15a_module)
{
    uint16 read_data = 0;
    ...
    if (menc15a_module == menc15a_1_module)
    {
        ...
        gpio_init(P02_5, GPI, GPIO_HIGH, GPI_PULL_UP); //MISO设置为浮空输入(硬件上将MOSI与MISO连接)
        MENC15A_1_CS(0);

        spi_write_8bit(MENC15A_1_SPI, 0x80);
        spi_write_8bit(MENC15A_1_SPI, 0x21);

        gpio_init(P02_6, GPI, GPIO_HIGH, GPI_PULL_UP); // MOSI设置为浮空输入
        read_data = spi_read_8bit(MENC15A_1_SPI);
        read_data = ((read_data << 8) | spi_read_8bit(MENC15A_1_SPI));
        spi_init(MENC15A_1_SPI, SPI_MODE1, MENC15A_1_SPI_SPEED, MENC15A_1_CLK_PIN, MENC15A_1_MOSI_PIN, MENC15A_1_MISO_PIN, SPI_CS_NULL);
        MENC15A_1_CS(1);
    }
    else
    {
        ...
        gpio_init(P10_1, GPI, GPIO_HIGH, GPI_PULL_UP);
        read_data = spi_read_8bit(MENC15A_2_SPI);
        read_data = ((read_data << 8) | spi_read_8bit(MENC15A_2_SPI));
        spi_init(MENC15A_2_SPI, SPI_MODE1, MENC15A_2_SPI_SPEED, MENC15A_2_CLK_PIN, MENC15A_2_MOSI_PIN, MENC15A_2_MISO_PIN, SPI_CS_NULL);
        MENC15A_2_CS(1);
    }
